  ...
  const processImage = async () => {
    ...
    try {
      ...
      if (processedData.length === 0) {
        throw new Error('Không tìm thấy dữ liệu nhân viên trong ảnh.');
      }

      onDataExtracted(processedData);
      setSuccessMessage(`✅ Đã trích xuất thành công ${processedData.length} nhân viên!`);
      ...
      let errorMessage = err.message || 'Không thể xử lý ảnh.';
      if (errorMessage.includes('JSON')) {
        errorMessage = 'Không thể đọc dữ liệu từ ảnh. Vui lòng chụp lại ảnh rõ hơn.';
      }
      ...
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800 flex items-center">
            <Camera size={20} className="mr-2" /> Chụp Ảnh & Trích Xuất
          </h2>
